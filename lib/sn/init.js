 ...
 */
sn.chdir = function( dir, callback ) {
	try {
		process.chdir( dir );
...
/**
 * Handles exiting the current process with notice.
 *
 * @return  {Boolean}  FALSE
 */
sn.exitError = function( err ) {
	if ( typeof err !== 'undefined' ) {
		sn.utils.error( err );
	} // if()

	sn.utils.info( 'Exiting...' );
	process.exit(1);
}; // sn.exitError()
...
sn.utils.debug = function( msg ) {
	if ( ! sn.options.cli.args.debug ) {
		return false;
	} // if()

	console.info( msg.debug );
...
sn.options.getConfig = function() {
	var fs         = require('fs');
	var configPath = sn.homeDir + '/snifter.json';

	if ( ! fs.existsSync(configPath) ) {
		sn.exitError( 'snifter.json can not be found at ' + configPath + ' run sn --config' );
...
/**
 * Sets config.theme.create.themeSlug option from Theme Name.
 *
 * @since   1.0.0
 *
 * @return  {Boolean}  FALSE
 */
sn.options.set.themeSlug = function() {
	if ( ! sn.options.config.theme.isNewTheme ) {
		return false;
	} // if()

	var title = ( typeof sn.options.config.theme.create.themeName === 'undefined' ) ? '' : sn.options.config.theme.create.themeName;

	if ( title === '' || sn.options.config.theme.create.themeSlug !== '' ) {
		return false;
	} // if()

	sn.options.config.theme.create.themeSlug = sn.options.config.theme.create.themeName.convertToSlug();

	return false;
}; // sn.options.set.themeSlug()
 ...
 * @return  {Boolean}  FALSE
 */
sn.options.cli.setArguments = function() {
	var program       = require('commander');
	var pjson         = require(sn.path + '/package.json');

	program
	.version(pjson.version)
	.option('-i, --install', 'Install all the things!')
	.option('-c, --config', 'Create a configuration file in ' + sn.homeDir)
	.option('-d, --debug', 'Outputs debug information')
	.parse(process.argv);

	args                = {};
	args.install        = ( typeof program.install !== 'undefined' );
	args.config         = ( typeof program.config !== 'undefined' );
	args.debug          = ( typeof program.debug !== 'undefined' );
	sn.options.cli.args = args;

	return false;
}; // sn.options.cli.setArguments()
 ...
 * @return  {Boolean}  FALSE
 */
sn.exec.createConfig = function() {
	var fs            = require('fs');
	var configFile    = 'snifter.json';
	var configPath    = sn.homeDir + '/' + configFile;
	...
	// We do not want to overwrite the configuration if it already exists
	if ( configExists ) {
		sn.utils.error( 'Configuration file already exists ' + configPath );
		return false;
	} // if()

	// Get default configuration
	var defaultPath    = sn.path + '/' + configFile;
	var defaultOptions = {
		encoding : 'utf8',
	};
	var defaultConfig = fs.readFileSync(defaultPath, defaultOptions);

	/// Write configuration file
	fs.writeFileSync( configPath, defaultConfig, configOptions);

	return false;
}; // sn.exec.createConfig()

/**
 * Handles the required install steps.
 *
 * @param   {Function}  callback  The function to be executed on completion.
 *
 * @return  {Boolean}            FALSE
 */
sn.exec.handleInstall = function( callback ) {
...
sn.init = function( callback ) {
	...
	// Set home directory
	sn.homeDir = process.env[(process.platform == 'win32') ? 'USERPROFILE' : 'HOME'];
	...
	// If we are just creating a configuration file we should just exit now
	if ( sn.options.cli.args.config ) {
		sn.exec.createConfig();
	...
	// Handle Install
	if ( sn.options.cli.args.install ) {
		sn.exec.handleInstall( callback );
